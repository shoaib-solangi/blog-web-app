          ...
          <div className={`flex flex-col md:flex-row gap-4 items-center ${menuOpen ? 'block' : 'hidden'} md:flex`}>
            ...
            {user ? (
              <>
                {dataUser === "admin" && (
                  <Link href="/PostBlog">
                    <Button className="text-white bg-blueGray-900 hover:bg-white hover:text-black px-4 py-2">Post Blog</Button>
                  </Link>
                )}
                <Button onClick={() => {LogOut()}} className="text-white bg-blueGray-900 hover:bg-white hover:text-black px-4 py-2">Logout</Button>
              </>
            ) : (
              <Link href="/Login">
                <Button className="text-white bg-blueGray-900 hover:bg-white hover:text-black px-4 py-2">Login</Button>
              </Link>
            )}
